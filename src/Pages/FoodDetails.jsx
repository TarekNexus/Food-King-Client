...
const { user } = useContext(AuthContext);
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 bg-red-50 shadow-md rounded-lg border border-red-200">
      ...
      <p className="text-lg text-red-800"><strong>Quantity:</strong> {food.quantity}</p>
      <p className="text-lg text-red-800"><strong>Location:</strong> {food.location}</p>
      <p className="text-lg text-red-800"><strong>Expires At:</strong> {new Date(food.expiredAt).toLocaleString()}</p>
      <p className="text-lg text-red-800"><strong>Status:</strong> {food.status}</p>

      <p className="text-md mt-3 text-gray-800"><strong className="text-red-600">Notes:</strong> {food.notes}</p>
      {food.additionalNotes && (
        <p className="text-sm text-gray-600"><strong className="text-red-500">Additional Notes:</strong> {food.additionalNotes}</p>
      )}

      {/* Donor Info */}
      <div className="mt-6 pt-4 border-t border-red-200 flex items-center gap-4">
        <img src={food.donorImage} alt={food.donorName} className="w-16 h-16 rounded-full object-cover border-2 border-red-300" />
        <div>
  ...
  <div className="fixed inset-0 z-50 bg-black/30 backdrop-blur-sm flex items-center justify-center">
    <div className="bg-white p-6 rounded-lg w-full max-w-lg shadow-lg border">
      <h3 className="text-xl font-bold mb-4 text-center text-red-700">Request Food</h3>
